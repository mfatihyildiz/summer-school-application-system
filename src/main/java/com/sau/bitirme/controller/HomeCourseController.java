...
public class HomeCourseController {
    ...
    @GetMapping("/{id}")
    public HomeCourse getHomeCourseById(@PathVariable Long id) {
        return homeCourseRepo.findById(id)
                .orElseThrow(() -> new RuntimeException("ExternalCourse not found with id: " + id));
    }

    @PostMapping
    public HomeCourse createHomeCourse(@RequestBody HomeCourse homeCourse) {
        return homeCourseRepo.save(homeCourse);
    }

    @DeleteMapping("/{id}")
    public void deleteHomeCourse(@PathVariable Long id) {
